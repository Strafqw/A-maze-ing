...
def main():
    try:
        config = Confs().read_confs()

        # Mark FT cells as "visited" (blocked) so DFS won't carve into them
        if config.get('WIDTH') < 9 or config.get('HEIGHT') < 7:
            print(MazeError("Dimensions too short to print 42 symbol"))
        maze = MazeGenerator(config)
        if config.get('PERFECT') == False:
            for i in range(1000):
        ...
        path = maze.bfs_path()
        # print(path)
        maze.export_maze_hex("output_maze.txt")
