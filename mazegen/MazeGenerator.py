"""Maze generation, solving and export.

Defines :class:`MazeGenerator`, which carves a maze with an iterative
depth-first search (recursive backtracker), optionally makes it
imperfect, solves it with BFS and exports it to the hex wall format.
"""

import random
from typing import Optional

from .vars import DIRECTIONS, EAST, GLYPH_42, NORTH, OPPOSITE, WEST


class MazeError(Exception):
    """Raised when a maze cannot be generated or exported."""

    def __init__(self, message: str) -> None:
        super().__init__(message)


class MazeGenerator:
    """Generate, solve and export a maze.

    Args:
        width: Maze width in cells.
        height: Maze height in cells.
        entry: Entry coordinates ``(x, y)``.
        exit: Exit coordinates ``(x, y)``.
        perfect: If ``True``, the maze has a single path between any
            two cells. If ``False``, extra openings add loops.
        seed: Optional seed for reproducible generation.
    """

    def __init__(
        self,
        width: int,
        height: int,
        entry: tuple[int, int],
        exit: tuple[int, int],
        perfect: bool = True,
        seed: Optional[int] = None,
    ) -> None:
        self.width = width
        self.height = height
        self.entry = entry
        self.exit = exit
        self.perfect = perfect
        self.seed = seed
        self.ft_grid: list[list[bool]] = []
        self.solution: Optional[list[tuple[int, int]]] = None
        self.small_size: bool = False
        self.grid = self.dfs(self.width, self.height)
        self.make_imperfect()

    def close_ft(
        self, visited: list[list[bool]], glip: list[tuple[int, int]]
    ) -> list[list[bool]]:
        """Mark the cells of the "42" glyph as blocked (visited).

        On large enough mazes the glyph is centered; the entry and exit
        must not fall on it. On smaller mazes the glyph is skipped and
        ``small_size`` is set.
        """
        if self.width >= 9 and self.height >= 7:
            for gx, gy in glip:
                row = gy + self.height // 2 - 2
                col = gx + self.width // 2 - 3
                visited[row][col] = True
            self.ft_grid = [row[:] for row in visited]
            if self.ft_grid[self.entry[1]][self.entry[0]]:
                raise MazeError("entry is in 42 symbol")
            if self.ft_grid[self.exit[1]][self.exit[0]]:
                raise MazeError("exit is in 42 symbol")
        else:
            self.small_size = True
            self.ft_grid = [row[:] for row in visited]
        return visited

    def dfs(self, width: int, height: int) -> list[list[int]]:
        """Carve a perfect maze with an iterative DFS backtracker."""
        # creates all cells with 4 walls
        grid = [[0b1111 for _ in range(width)] for _ in range(height)]
        visited = [[False] * width for _ in range(height)]
        self.close_ft(visited, GLYPH_42)
        ex, ey = self.entry
        stack = [(ex, ey)]
        visited[ey][ex] = True
        if self.seed is not None:
            random.seed(self.seed)

        while stack:
            x, y = stack[-1]            # last visited cell
            found = False

            dirs = DIRECTIONS[:]
            random.shuffle(dirs)        # shuffle to vary the maze

            for direction, dx, dy in dirs:
                nx, ny = x + dx, y + dy

                inside = 0 <= nx < width and 0 <= ny < height
                if inside and not visited[ny][nx]:
                    grid[y][x] &= ~direction
                    grid[ny][nx] &= ~OPPOSITE[direction]

                    visited[ny][nx] = True
                    stack.append((nx, ny))  # go deeper
                    found = True
                    break

            if not found:
                stack.pop()  # backtrack

        return grid

    def between_four_nodes(self, x: int, y: int) -> bool:
        """Return ``True`` if (x, y) is an interior cell."""
        if self.width <= 2 or self.height <= 2:
            return True
        if 0 < x < self.width - 1 and 0 < y < self.height - 1:
            return True
        return False

    def make_imperfect(self) -> None:
        """Open one extra wall per row to introduce loops."""
        if self.perfect:
            return None
        dirs = DIRECTIONS[:]

        for y in range(self.height):
            placed = True
            for x in range(self.width):
                for direction, dx, dy in dirs:
                    nx = x + dx
                    ny = y + dy

                    inside = 0 <= nx < self.width and 0 <= ny < self.height
                    if inside and placed:
                        free = (not self.ft_grid[ny][nx]
                                and not self.ft_grid[y][x])
                        if free and self.between_four_nodes(x, y):
                            placed = False
                            self.grid[y][x] &= ~direction
                            self.grid[ny][nx] &= ~OPPOSITE[direction]

    def is_perfect_maze(
        self, blocked: Optional[list[list[bool]]] = None
    ) -> bool:
        """Return ``True`` if the maze is connected and acyclic.

        Args:
            blocked: Optional 2D bool grid (``True`` = ignore cell),
                e.g. :attr:`ft_grid`.
        """
        h, w = len(self.grid), len(self.grid[0])
        sx, sy = self.entry

        if blocked is None:
            blocked = [[False] * w for _ in range(h)]

        if not (0 <= sx < w and 0 <= sy < h) or blocked[sy][sx]:
            return False

        visited: set[tuple[int, int]] = set()
        stack: list[tuple[int, int, Optional[tuple[int, int]]]] = [
            (sx, sy, None)
        ]

        while stack:
            x, y, parent = stack.pop()

            if (x, y) in visited:
                # revisited through another route => cycle
                return False

            visited.add((x, y))
            cell = self.grid[y][x]

            for direction, dx, dy in DIRECTIONS:
                if (cell & direction) == 0:  # passage is open
                    nx, ny = x + dx, y + dy
                    if 0 <= nx < w and 0 <= ny < h and not blocked[ny][nx]:
                        if parent is not None and (nx, ny) == parent:
                            continue  # ignore the edge back to parent
                        stack.append((nx, ny, (x, y)))

        return True

    def solve(
        self, blocked: Optional[list[list[bool]]] = None
    ) -> Optional[list[tuple[int, int]]]:
        """Find the shortest path from entry to exit with BFS.

        Returns:
            The path as a list of ``(x, y)`` cells, or ``None`` if the
            exit is unreachable.
        """
        w, h = self.width, self.height
        sx, sy = self.entry
        gx, gy = self.exit

        if blocked is None:
            blocked = [[False] * w for _ in range(h)]

        def ok(x: int, y: int) -> bool:
            return 0 <= x < w and 0 <= y < h and not blocked[y][x]

        if not ok(sx, sy) or not ok(gx, gy):
            return None

        queue: list[tuple[int, int]] = [(sx, sy)]
        head = 0
        parent: dict[tuple[int, int], Optional[tuple[int, int]]] = {
            (sx, sy): None
        }

        while head < len(queue):
            x, y = queue[head]
            head += 1

            if (x, y) == (gx, gy):
                break

            cell = self.grid[y][x]
            for wall_bit, dx, dy in DIRECTIONS:
                # open if wall bit not set
                if (cell & wall_bit) == 0:
                    nx, ny = x + dx, y + dy
                    if ok(nx, ny) and (nx, ny) not in parent:
                        parent[(nx, ny)] = (x, y)
                        queue.append((nx, ny))

        if (gx, gy) not in parent:
            return None

        # reconstruct
        path: list[tuple[int, int]] = []
        cur: Optional[tuple[int, int]] = (gx, gy)
        while cur is not None:
            path.append(cur)
            cur = parent[cur]
        path.reverse()
        self.solution = path
        return path

    def path_to_dirs(self) -> str:
        """Convert the solution path to a 'NESW' string."""
        if not self.solution or len(self.solution) < 2:
            return ""

        out = []
        for (x1, y1), (x2, y2) in zip(self.solution, self.solution[1:]):
            dx, dy = x2 - x1, y2 - y1
            if dx == 0 and dy == -1:
                out.append("N")
            elif dx == 1 and dy == 0:
                out.append("E")
            elif dx == 0 and dy == 1:
                out.append("S")
            elif dx == -1 and dy == 0:
                out.append("W")
            else:
                raise ValueError(
                    f"Invalid step: {(x1, y1)} -> {(x2, y2)}"
                )
        return "".join(out)

    def export(self, out_file: str) -> None:
        """Write the maze to ``out_file`` in the hex wall format.

        One hex digit per cell (closed-wall bits N=1, E=2, S=4, W=8),
        rows one per line; then a blank line, the entry and exit
        coordinates, and the solution path. Every line ends with ``\\n``.
        """
        with open(out_file, "w", encoding="utf-8") as f:
            for y in range(self.height):
                line = "".join(
                    format(self.grid[y][x] & 0xF, "X")
                    for x in range(self.width)
                )
                f.write(line + "\n")

            f.write("\n")
            f.write(",".join(str(el) for el in self.entry))
            f.write("\n")
            f.write(",".join(str(el) for el in self.exit))
            f.write("\n")
            f.write(self.path_to_dirs())


# just to visualize and know if it is working
def draw_ascii(grid: list[list[int]]) -> None:
    """Print a simple ASCII rendering of a wall-bitmask grid."""
    h = len(grid)
    w = len(grid[0])

    for y in range(h):
        # top walls
        top = ""
        mid = ""

        for x in range(w):
            cell = grid[y][x]

            top += "┼───" if cell & NORTH else "┼   "

            mid += "│   " if cell & WEST else "    "

            if x == w - 1:
                mid += "│" if cell & EAST else " "

        print(top + "┼")
        print(mid)

    print("┼" + "───┼" * w)


def main() -> None:
    """Quick manual check of the generator."""
    maze = MazeGenerator(
        width=10, height=8, entry=(0, 0), exit=(1, 4), perfect=True
    )
    if not maze.perfect:
        for _ in range(1000):
            if not maze.is_perfect_maze():
                break
            maze = MazeGenerator(
                width=9, height=7, entry=(0, 0), exit=(8, 6),
                perfect=True, seed=None,
            )

    draw_ascii(maze.grid)
    maze.solve()


if __name__ == '__main__':
    main()
